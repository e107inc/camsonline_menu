...
// e107::plugLan('pm', null);
$folder = "camsonline_menu";
$text = "";

if(file_exists(THEME . "images/bullet2.gif"))
{
	define('BULLET', "<img src='" . THEME . "images/bullet2.gif' alt='bullet' />");
}
else
{
	define('BULLET', '&middot;');
}
...
if(USER == true || ADMIN == true)
{
	...
	list($uid, $upw) = (isset($_COOKIE[$pref['cookie_name']]) ? explode(".", $_COOKIE[$pref['cookie_name']]) : explode(".", $_SESSION[$pref['cookie_name']]));
	$sql = new db;
	if($sql->select("user", "*", "user_id='$uid' AND md5(user_password)='$upw'"))
	{
		if(ADMIN == true)
		{
			...
			$text .= BULLET." <a href='" . e_ADMIN . $adminfpage . "'>" . LOGIN_MENU_L11 . "</a><br />";
		}
	...
	/*	if(!$sql->db_Select("online", "*", "online_ip='$ip' AND online_user_id='0' "))
		...
		if(!$new_chat)
		{
			$new_chat = LOGIN_MENU_L26;
		}
		$new_forum = $sql->isTable('forum_t') && $sql->db_Count("forum_t", "(*)", "WHERE thread_datestamp>'" . $time . "' ");
		if(!$new_forum)
		{
		...
		$text .= "<div style='cursor:hand' onClick=\"expandit('guests')\"><table style='width:95%'><tr ><td class='smalltext' ><a><b>Guests Online</b></a></td><td class='smalltext' style='text-align:right'><b><a  >" . GUESTS_ONLINE . "</a></b></td></tr></table></div>";
		...
		if(ADMIN)
		{
			...
			while($row = $sql->db_Fetch())
			{
				...
				{
					$online_location = "#";
					$online_location_page = ":P";
					$spanit = "style='color:red'";
				}
				else
				{
					$spanit = "";
		...
		if($pref['user_reg'] == 1)
		{
			$text .= "<div style='cursor:hand' title='View members online' onClick=\"expandit('members')\"><table style='width:95%'><tr><td class='smalltext' ><a title='View members online'><b>Members Online</b></a></td><td class='smalltext' style='text-align:right'><b><a title='View members online'>" . MEMBERS_ONLINE . "</a></b></td></tr></table></div>";
		}
...
//PM System Start
	...
	";
		...
		if(check_class($pm_prefs['pm_class']))
		{
			...
			$pm_shortcodes = e107::getScBatch('pm_shortcodes');
...
//PM System End
		...
		$text .= "<br /><div style='text-align:center; font-weight:bold'><a style='cursor:hand' onClick='expandit(this)' >Stat Info.</a></div>";
